...
export interface RecapData { roleLabel: string; imposter: string; secretLabel: string; secret: string; verdict: string; rowsTitle?: string; rows?: RecapRow[]; }
...
export async function renderRecap(data: RecapData): Promise<Blob> {
  ...
  const bg = c.createRadialGradient(540, 520, 60, 540, 700, 1500); bg.addColorStop(0, '#2d5442'); bg.addColorStop(.5, '#12302a'); bg.addColorStop(1, '#08161a');
  c.fillStyle = bg; c.fillRect(0, 0, width, height);
  c.strokeStyle = '#cfb17b44'; c.lineWidth = 2; c.strokeRect(60, 60, width - 120, height - 120); c.strokeRect(78, 78, width - 156, height - 156);
  c.textAlign = 'center'; c.textBaseline = 'alphabetic';
  const eyebrow = (text: string, y: number, color = '#cfb17b') => { c.fillStyle = color; c.font = `500 30px ${sans}`; c.letterSpacing = '8px'; c.fillText(text, 540, y); c.letterSpacing = '0px'; };
  eyebrow(`IMPOSTER  ·  ${siteHost.toUpperCase()}`, 190);
  eyebrow(data.roleLabel, 420, '#b4c2b7');
  c.fillStyle = '#f3ecdd'; const nameSize = fit(c, data.imposter, s => `600 ${s}px ${serif}`, 190, 900, 90); c.font = `600 ${nameSize}px ${serif}`; c.fillText(data.imposter, 540, 600);
  c.fillStyle = '#cfb17b'; c.fillRect(490, 660, 100, 2);
  eyebrow(data.secretLabel, 760, '#b4c2b7');
  c.fillStyle = '#edd4a0'; c.font = `500 84px ${serif}`; const secretLines = wrap(c, data.secret, 880).slice(0, 3); let y = 860;
  ...
  if (data.rows?.length) {
    ...
    for (const row of rows) {
      y += rowH; c.textAlign = 'left'; c.font = `${row.highlight ? 600 : 500} ${size}px ${sans}`; c.fillStyle = row.highlight ? '#edd4a0' : '#d6d9c9'; c.fillText(row.highlight ? `✦ ${row.label}` : row.label, 160, y);
      c.textAlign = 'right'; c.font = `500 ${size}px ${serif}`; c.fillStyle = row.highlight ? '#edd4a0' : '#f3ecdd';
      ...
      c.fillStyle = '#cfb17b22'; c.fillRect(160, y + 18, 760, 1);
    }
    c.textAlign = 'center'; y += 80;
  }
  c.fillStyle = '#f3ecdd'; c.font = `400 46px ${serif}`; for (const line of wrap(c, data.verdict, 860).slice(0, 2)) { c.fillText(line, 540, Math.max(y, 1560)); y = Math.max(y, 1560) + 56; }
