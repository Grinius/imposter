...
export type Action = { type: 'reveal' | 'hide' | 'privacy' | 'start-vote' | 'open-ballot' | 'skip-guess' } | { type: 'vote'; target: number } | { type: 'guess'; word: string };
...
export function createRound(settings: Settings, random: () => number, previousWord?: string): Round {
  ...
  return { names: normalized.names, settings: normalized, word: pool[pick(pool.length, random)], imposter: pick(settings.names.length, random), firstClue: pick(settings.names.length, random), phase: 'handoff', cursor: 0, votes: [], accused: null, winner: null, reason: null };
}
