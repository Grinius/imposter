...
export default function UpgradeCard({ feature, compact = false, unlocked = false, available = false }: { feature?: PremiumFeature; compact?: boolean; unlocked?: boolean; available?: boolean }) {
  ...
  return <aside className={`upgrade-card ${compact ? 'compact' : ''} ${unlocked ? 'unlocked' : ''}`} aria-label={`${title} is a ${unlocked ? 'premium feature you’ve unlocked' : 'premium feature'}`}>
    ...
    <h2>{title}</h2>
    <p>{summary}</p>
    {unlocked ? <span className="upgrade-note">{active ? 'Thanks for going premium — this one\'s active.' : 'Thanks for going premium — this one\'s still on the way.'}</span> : premiumConfigured ? <a className="gold-button" href={stripePaymentLink}>Unlock with Stripe <ArrowRight size={17} /></a> : <Link className="outline-button" href="/premium/"><Sparkles size={16} /> View premium options</Link>}
  </aside>;
}
