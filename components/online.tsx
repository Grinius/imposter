...
import { ArrowLeft, ArrowRight, Check, ChevronDown, Clock3, Copy, Link2, LockKeyhole, Radio, Users, Wifi, X } from 'lucide-react';
...
export default function OnlineGame() {
  ...
  useEffect(() => { const savedCode = window.localStorage.getItem('imposter-room-code'); if (savedCode) connect(savedCode, readSeat(savedCode)); return () => socket.current?.close(); }, []);
  ...
  function leaveRoom() { leaving.current = true; socket.current?.close(); socket.current = null; window.localStorage.removeItem('imposter-room-code'); setRoom(null); setRole(null); setError(''); setRoomCode(''); setClue(''); setGuess(''); setMyPlayerId(''); }
