...
import { ArrowLeft, ArrowRight, Brain, Briefcase, Check, ChevronDown, CircleHelp, Clapperboard, Clock3, Coffee, Eye, EyeOff, Feather, Fingerprint, Flag, Gem, Ghost, Gift, Heart, LockKeyhole, MapPin, Martini, MessageCircleQuestion, MicVocal, Music, PartyPopper, Pause, PawPrint, PenLine, Play, Plus, RotateCcw, ShieldCheck, ShoppingBag, Shuffle, Smartphone, Sparkles, Swords, Timer, TreePine, Trophy, type LucideIcon, Users, Utensils, Volume2, VolumeX, Vote, Wifi, X } from 'lucide-react';
...
const categoryIcons: Record<Category, LucideIcon> = { mixed: Shuffle, food: Utensils, animals: PawPrint, everyday: Coffee, places: MapPin, objects: Gem, activities: Sparkles, 'date-night': Heart, holidays: Gift, halloween: Ghost, football: Trophy, 'k-pop': MicVocal, 'pop-superstars': Music, christmas: TreePine, brainrot: Brain, 'night-out': Martini, office: Briefcase, 'american-football': Flag, bachelorette: PartyPopper, anime: Swords, brands: ShoppingBag, movies: Clapperboard };
const featuredThemedCount = 6;
...
export default function Game() {
  ...
  // The themed row shows two tidy rows of three by default; the rest sit behind a toggle. A pack
  // arriving via ?pack= (from its /packs/ page) that is not in the featured six expands the row.
  const [showAllThemed, setShowAllThemed] = useState(false);
  ...
  function start(replay = false) {
    ...
    const next = createRound(settings, secureRandom, recentWords(), { maxPlayers, premium }); rememberWord(next.word.id);
    setRound(next); track({ name: 'round_start', mode: 'word', pack: settings.category, players: settings.names.length });
  ...
  return <>
    ...
    <header className="site-header">
      <Link className="brand" href="/" onClick={event => { if (round) { event.preventDefault(); setExitOpen(true); } }} aria-label="Imposter home"><Emblem small /><BrandWordmark /></Link>
      ...
      {isSetup ? <div className="setup-layout">
        ...
        <section className="setup-panel" aria-labelledby="setup-title">
          ...
          <form onSubmit={event => { event.preventDefault(); start(); }}>
            ...
            {(['core', 'themed'] as const).map(group => { const items = categories.filter(item => item.group === group); const collapsed = group === 'themed' && !showAllThemed && !items.slice(featuredThemedCount).some(item => item.id === settings.category); const shown = collapsed ? items.slice(0, featuredThemedCount) : items; return <div key={group}>{group === 'themed' && <div className="field-heading themed-heading"><label id="themed-label">Themed packs <Link href="/packs/">All packs <ArrowRight size={12} /></Link></label></div>}
            <div className="category-grid" role="group" aria-labelledby={group === 'core' ? 'category-label' : 'themed-label'}>{shown.map(item => { const Icon = categoryIcons[item.id]; const locked = item.premium && !premium; return <button type="button" className={`category-button ${settings.category === item.id ? 'selected' : ''} ${locked ? 'premium-slot' : ''}`} key={item.id} aria-pressed={settings.category === item.id} onClick={() => { setSettings({ ...settings, category: item.id as Category }); setCategoryPicked(true); setPaywallReasons(null); }}><Icon size={19} strokeWidth={1.5} /><span>{item.short}</span>{locked && <small className="premium-tag"><LockKeyhole size={10} /> Premium</small>}{settings.category === item.id && <Check size={11} className="category-check" />}</button>; })}</div>
            {group === 'themed' && items.length > featuredThemedCount && <button type="button" className="themed-toggle" aria-expanded={!collapsed} onClick={() => setShowAllThemed(!!collapsed)}>{collapsed ? <>See all {items.length} packs <ChevronDown size={13} /></> : <>Show fewer packs <ChevronDown size={13} className="flipped" /></>}</button>}</div>; })}
