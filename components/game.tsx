    ...
    <main id="game">
      {isSetup ? <div className="setup-layout">
        <section className="intro">
          ...
          <div className="intro-facts"><span><Users size={16} /> 3–12 friends</span><span><Clock3 size={16} /> 5-minute rounds</span><span><Smartphone size={16} /> Just one phone</span></div>
        </section>
        <section className="setup-panel" aria-labelledby="setup-title">
          ...
          <form onSubmit={event => { event.preventDefault(); start(); }}>
            <div className="field-heading"><label id="players-label"><span className="step-number">01</span> Who’s at the table?</label><span className="count-label">{settings.names.length} / 12 players</span></div>
            <div className="player-inputs" role="group" aria-labelledby="players-label">{settings.names.map((name, index) => <div className="player-input" key={index}><Avatar index={index} name={name} /><input aria-label={`Player ${index + 1} name`} value={name} maxLength={20} autoComplete="off" onChange={event => { setSettings({ ...settings, names: settings.names.map((value, i) => i === index ? event.target.value : value) }); setError(''); }} /><button type="button" aria-label={`Remove player ${index + 1}`} disabled={settings.names.length <= 3} onClick={() => setSettings({ ...settings, names: settings.names.filter((_, i) => i !== index) })}><X size={14} /></button></div>)}</div>
            <button className="add-player" type="button" disabled={settings.names.length >= 12} onClick={() => { let number = settings.names.length + 1; while (settings.names.includes(`Player ${number}`)) number++; setSettings({ ...settings, names: [...settings.names, `Player ${number}`] }); }}><Plus size={15} /> Add a player</button>
            <div className="field-heading category-heading"><label id="category-label"><span className="step-number">02</span> Pick your secret world.</label></div>
            ...
            <div className="setup-links"><a className="online-link" href="/online/"><Wifi size={16} /> Play online with friends <ArrowRight size={15} /></a><a className="online-link" href="/imposter-game-generator/"><Shuffle size={16} /> Open game generator <ArrowRight size={15} /></a><a className="online-link" href="/imposter-game-rules/"><CircleHelp size={16} /> Read rules <ArrowRight size={15} /></a></div>
