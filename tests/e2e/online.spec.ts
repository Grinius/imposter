...
test('three browsers complete a private online round', async ({ browser }) => {
  ...
  const imposterPage = pages[imposterIndex]; await expect(imposterPage.getByPlaceholder('Guess the secret word')).toBeVisible({ timeout: 15_000 }); await imposterPage.getByPlaceholder('Guess the secret word').fill('definitely-wrong-guess'); await imposterPage.getByRole('button', { name: /Make final guess/i }).click(); for (const page of pages) await expect(page.locator('.reveal-stage')).toBeVisible({ timeout: 15_000 });
  // Every phone gets the reveal only now: the imposter's seat and the word were never broadcast before the result.
  const broadcasts = await pages[0].evaluate(() => (window as Window & { __rooms?: { game?: { phase: string; reveal?: unknown } }[] }).__rooms ?? []);
