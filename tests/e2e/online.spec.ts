...
test('three browsers complete a private online round', async ({ browser }) => {
  ...
  for (let turn = 0; turn < names.length; turn += 1) { let acted = false; for (const page of pages) { if (await page.getByPlaceholder('Your clue').isVisible().catch(() => false)) { await waitForTurn(page); await page.getByPlaceholder('Your clue').fill(`clue ${turn + 1}`); await page.getByRole('button', { name: /Submit clue/i }).click(); acted = true; break; } } expect(acted).toBe(true); }
